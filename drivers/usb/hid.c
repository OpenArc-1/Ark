...
void hid_describe_report(const hid_boot_report_t *r) {
    if (!r) return;
    printk(T, "hid: mods=0x%02x keys: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
           (u32)r->modifiers,
           (u32)r->keycode[0], (u32)r->keycode[1], (u32)r->keycode[2],
           (u32)r->keycode[3], (u32)r->keycode[4], (u32)r->keycode[5]);
}
