...
/* human‑readable size helper (no stdlib) */
static void fmt_size(u32 bytes, char *buf, u32 buflen) {
    ...
    if (v == 0) {
        ...
        while (v > 0 && ni < (int)sizeof(num)-1) {
            num[ni++] = '0' + (v % 10);
            v /= 10;
        }
    }
    for (int i = 0; i < ni/2; i++) {
        char t = num[i]; num[i] = num[ni-1-i]; num[ni-1-i] = t;
    }
    num[ni] = '\0';
    int pos = 0;
    for (int i = 0; num[i] && pos+1 < (int)buflen; i++) buf[pos++] = num[i];
...
static void ata_identify(u8 bus, u8 drive) {
    ...
    {
        char hsz[32];
        fmt_size(dev->lba_sectors * 512U, hsz, sizeof(hsz));
        printk(T,"ata: Found device at bus %d drive %d: %u sectors (%s)\n", 
               bus, drive, dev->lba_sectors, hsz);
    }
}
