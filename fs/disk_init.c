...
/* Human-readable size formatting (simple, no stdlib)                 */
...
static void fmt_size(u32 bytes, char *buf, u32 buflen) {
    ...
    /* compose output */
    int pos = 0;
    for (int i = 0; numbuf[i] && pos+1 < (int)buflen; i++)
        buf[pos++] = numbuf[i];
    if (pos+1 < (int)buflen)
        buf[pos++] = ' ';
    for (int i = 0; unit[i] && pos+1 < (int)buflen; i++)
        buf[pos++] = unit[i];
    buf[pos] = '\0';
}
...
static u32 scan_dir_sector(const u8 *sec) {
    for (int i = 0; i < 512; i += 32) {
...

        {
            u32 fsize = le32(e + 28);
            char hsz[32];
            fmt_size(fsize, hsz, sizeof(hsz));
            printk(T,"disk: found INIT file, cluster=%u size=%u (%s)\n",
                   cluster, fsize, hsz);
        }
...
static int try_fat(u32 partition_lba) {
    ...
    {
        char hsz[32];
        fmt_size(bytes, hsz, sizeof(hsz));
        printk(T,"disk: loaded .init (%u bytes, %s) -> /init\n", bytes, hsz);
    }
...
static int try_mbr(void) {
    ...
    for (int p = 0; p < 4; p++) {
         ...
         *            0x0E=FAT16 LBA, 0x83= (try anyway) */
        {
            char hsz[32];
            fmt_size(size * 512U, hsz, sizeof(hsz)); /* rough bytes assuming 512-byte sectors */
            printk(T,"disk: partition %d type=0x%x lba=%u size=%u (%s)\n",
                   p, type, lba, size, hsz);
        }
...
int disk_load_init(void) {
    ...
    /* ── Try AFS (Ark File System) first — GPT and MBR ── */
    printk(T,"disk: probing AFS partitions...\n");
    if (afs_disk_probe()) {
        printk(T,"disk: /init loaded from AFS partition\n");
        return 1;
    }
