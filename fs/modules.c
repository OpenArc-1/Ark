/**
 * Multiboot Module Loader -- -like initramfs support
 *
 * Boot flow (mirrors  initramfs):
...
u32 modules_load_from_multiboot(multiboot_info_t *mbi) {
    ...
    for (u32 i = 0; i < mod_count; ++i) {
        ...
        if (size >= 4 && data[0] == 'P' && data[1] == 'K') {
            printk(T, "initramfs: module %u — ZIP initramfs from GRUB/multiboot @ 0x%x (%u bytes)\n",
                   i + 1, size, start);

#if CONFIG_ZIP_ENABLE
            u32 n = zip_load_into_ramfs(data, size);
            if (n > 0) {
                total_loaded += n;
                printk(T, "initramfs: extracted %u file(s) into ramfs\n", n);

                /* Warn if /init is missing -- like 's "No init found" panic */
...
#else
            printk(T, "initramfs: ZIP support disabled (CONFIG_ZIP_ENABLE=0), skipping module\n");
#endif
            continue;
        }
        ...
        printk(T, "initramfs: module %u — raw ELF/binary from bootloader -> %s (%u bytes @ 0x%x)\n",
