...
void print_disk_info(const DiskInfo *info) {
    ...
    if (info->total_gb_int > 0) {
        printk(T, "disk%u: %s/%s  %u.%u GB (%u MB)  [%s]",
               (u32)info->drive,
               bus_str[info->drive & 3], role_str[info->drive & 3],
               info->total_gb_int, info->total_gb_frac,
               info->total_mb,
               info->method == 1 ? "LBA48" : "LBA28");
    } else {
        printk(T, "disk%u: %s/%s  %u MB  [%s]",
               (u32)info->drive,
               bus_str[info->drive & 3], role_str[info->drive & 3],
               info->total_mb,
               info->method == 1 ? "LBA48" : "LBA28");
    ...
    if (info->sectors_hi)
        printk(T, "  sectors: 0x%x%08x\n", info->sectors_hi, info->sectors_lo);
    else
        printk(T, "  sectors: %u\n", info->sectors_lo);
}
...
void print_disk_summary(const DiskInfo *disks, int count) {
    if (count == 0) {
        printk(T, "disk: no storage devices detected\n");
        return;
    }
    for (int i = 0; i < 4; i++) {
        if (disks[i].valid)
    ...
    printk(T, "disk: total devices found: %d\n", count);
}

void scan_and_display_disks(void) {
    DiskInfo disks[4];
    printk(T, "disk: scanning ATA storage devices\n");
...
void scan_and_display_disks_simple(void) {
    DiskInfo disks[4];
    printk(T, "disk: scanning storage devices\n");
    int count = scan_all_disk(disks);
    if (count == 0) { printk(T, "disk: no devices found\n"); return; }
    printk(T, "disk: found %d device(s)\n", count);
    for (int i = 0; i < 4; i++)
