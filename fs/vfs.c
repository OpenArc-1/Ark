...
int vfs_mount(const char *fs_type, const char *device, const char *mount_point) {
    ...
    } else if (vfs_strcmp(fs_type, "afs") == 0) {
        return afs_mount(device);
    }
...
int vfs_open(const char *path) {
    for (u32 i = 0; i < MAX_OPEN_FILES; i++) {
        if (!open_files[i].valid) {
            ...
            /* try AFS (Ark File System) */
            if (afs_is_mounted()) {
                int afs_fd = afs_open(path);
                if (afs_fd >= 0) {
                    file->size = afs_file_size(afs_fd);
                    file->type = VFS_TYPE_REGULAR;
                    afs_close(afs_fd); /* AFS manages its own fd table */
                    return file->fd;
                }
            }
...
int vfs_write(int fd, const void *buffer, u32 size) {
    if (fd < 0 || fd >= MAX_OPEN_FILES || !open_files[fd].valid)
        return -1;
    /* Ramfs/FAT are read-only from VFS; stdout/stderr are handled in syscall layer */
    (void)buffer;
    (void)size;
    return -1;  /* EPERM / read-only */
}
...
u8 vfs_file_exists(const char *path) {
    return ramfs_file_exists(path) || ramfs_dir_exists(path);
}

u32 vfs_list_count(const char *path) {
    return ramfs_list_count(path);
}

u8 vfs_list_at(const char *path, u32 index, char *name_out, u32 name_max) {
    return ramfs_list_at(path, index, name_out, name_max);
}
...
int vfs_mknod(const char *path, u32 type, u32 major, u32 minor) {
    return ramfs_mknod(path, type, major, minor) ? 0 : -1;
}