...
u32 zip_load_into_ramfs(const u8 *data, u32 size) {
    ...
    for (u32 n = 0; n < cd_entries && pos + 46 <= size; n++) {
        ...
        /* Save central-dir filename pointer BEFORE advancing pos */
        const char *cd_fn = (const char *)&data[pos + 46];

        pos += 46 + fn_len + extra_len + comment_len;

        /* Skip directories (trailing / in filename, or zero length) */
        if (fn_len == 0 || cd_fn[fn_len - 1] == '/')
            continue;
        ...
        if (local_off + 30 > size) {
            printk(T, "zip: local header out of range at %u\n", local_off);
            continue;
        }
        if (data[local_off] != 0x50 || data[local_off + 1] != 0x4b ||
            data[local_off + 2] != 0x03 || data[local_off + 3] != 0x04) {
            printk(T, "zip: bad local header sig at %u\n", local_off);
            continue;
        }

        /* IMPORTANT: local extra_len (offset 28 in local header) can differ
         * from central directory extra_len (offset 30 in central header).
         * ZIP spec §4.3.7: "The extra field length in the local header may
         * be different from the corresponding value in the central header."
         * Always use the LOCAL extra_len for payload offset calculation. */
        u16 local_fn_len    = (u16)data[local_off + 26] | ((u16)data[local_off + 27] << 8);
        u16 local_extra_len = (u16)data[local_off + 28] | ((u16)data[local_off + 29] << 8);

        /* Use local fn_len for the filename (safer than central fn_len) */
        u16 use_fn_len = local_fn_len < fn_len ? local_fn_len : fn_len;

        if (local_off + 30 + local_fn_len + local_extra_len + compressed > size) {
            printk(T, "zip: entry data out of range\n");
            continue;
        }

        const char *fn = (const char *)&data[local_off + 30];

        normalize_path(fn, use_fn_len);

        /* Payload starts after local header + local fn + local extra */
        u32 payload_off = local_off + 30 + local_fn_len + local_extra_len;
