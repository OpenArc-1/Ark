...
void kernel_main(void) {
     ...
     * Show a brief splash (white background, centered black "Ark")
     * before any verbose kernel output. */
    if (vesa_is_ready()) {
        /* simple centered text splash */
        const char *splash = "Ark";
        u32 w = vesa_get_width();
        u32 h = vesa_get_height();
        u32 text_w = 8 * 3; /* 3 chars, 8 px per char */
        u32 x = (w > text_w) ? ((w - text_w) / 2) : 0;
        u32 y = (h > 16) ? ((h - 16) / 2) : 0;
        vesa_clear_screen(VESA_BLACK);
        for (u32 i = 0; splash[i]; ++i) {
            vesa_draw_char(x + i * 8, y, splash[i], VESA_BLACK, VESA_WHITE);
        }
        /* small visible delay (~200 ms) */
        delay(40000000);
    }
    vesa_clear_screen(VESA_BLACK);
    ...
    * framebuffer region (0xE0000000) causes an immediate page fault and
    ...
    /* SMBIOS */
if (smbios_init()) {
    printk("[SMBIOS] init OK\n");
    smbios_dump();
} else {
    printk("[SMBIOS] init FAILED - EPS not found\n");
}
    ...
    /* -- VESA userspace FB mapping + PS/2 mouse driver -- */
    {
        extern void ark_syscall_fb_map(void);
        extern void ps2_mouse_kernel_init(void);
        ark_syscall_fb_map();
        ps2_mouse_kernel_init();
        printk(T,"fb: framebuffer mapped to userspace 0xE0000000\n");
        printk(T,"mouse: PS/2 auxiliary port initialized\n");
    }
    /* ── Network ───────────────────────────────────────────────── */
    ...
    /* initialize dynamic module subsystem (it only allocates its pool) */
    dkm_init_kernel();
