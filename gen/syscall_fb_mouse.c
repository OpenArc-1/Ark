...
void ps2_mouse_kernel_init(void) {
    ...
    /* Read CCB and update carefully — preserve keyboard IRQ (bit 0) */
    _wait_write(); io_outb(PS2K_CMD, 0x20);
    if (!_wait_read()) return;
    u8 cfg = io_inb(PS2K_DATA);
    cfg |= 0x02;   /* enable IRQ12 for mouse */
    cfg |= 0x01;   /* preserve keyboard IRQ1 — must not clear this */
    cfg &= ~0x20;  /* clear aux port disable bit */
...
long ark_syscall_mouse_read(void) {
    /* Only consume bytes where bit5 of status = 1 (aux/mouse data).
     * If bit5=0 the byte is from the keyboard — leave it alone. */
    while (io_inb(PS2K_STATUS) & PS2_OBF) {
        u8 status = io_inb(PS2K_STATUS);
        if (!(status & PS2_OBF)) break;
        if (!(status & 0x20)) break;  /* bit5=0 → keyboard data, stop */
