...
#define ARK_INIT_API_VERSION 4  /* added scheduler API */  /* bump for new log APIs */
...
typedef struct ark_kernel_api {
    ...
    /* Execute an ELF binary from the VFS by path with argc/argv.
     * The kernel reads the file, loads it, and runs it.
     * Returns the exit code, or -1 on error. */
    int (*exec_elf)(const char *path, int argc, char **argv);

    /* ── Scheduler / Job control ─────────────────────────────────── */
    void (*sched_list_tasks)(void);
    void (*sched_get_stats)(void *out);           /* sched_stats_t*  */
    u32  (*sched_spawn)(const char *name,
                        void (*entry)(void *),
                        void *arg,
                        u8 priority,
                        u32 stack_size);
