...
typedef struct {
    ...
    int printk_enabled;  // 0/1
    int usb_enabled;     // 0/1
    int audio_enabled;   // 0/1
    int net_enabled;     // 0/1
    int debug_verbose;   // 0/1
    int fb_enabled;      // 0/1
    int fb_width;
    int fb_height;
    int fb_bpp;
...
static void draw_menu(WINDOW *win, int highlight, const ark_config_t *cfg) {
    ...
    for (int i = 0; i < ITEM_COUNT; ++i) {
        ...
        case ITEM_ARCH:
            ...
            break;
        case ITEM_PRINTK_ENABLE:
            mvwprintw(win, row, 2, "[%c]  Kernel logging (printk)",
                      cfg->printk_enabled ? 'X' : ' ');
            break;
        case ITEM_USB_ENABLE:
            mvwprintw(win, row, 2, "[%c]  USB subsystem",
                      cfg->usb_enabled ? 'X' : ' ');
            break;
        case ITEM_AUDIO_ENABLE:
            mvwprintw(win, row, 2, "[%c]  Audio drivers",
                      cfg->audio_enabled ? 'X' : ' ');
            break;
        case ITEM_NET_ENABLE:
            mvwprintw(win, row, 2, "[%c]  Networking stack",
                      cfg->net_enabled ? 'X' : ' ');
            break;
        case ITEM_DEBUG_VERBOSE:
            mvwprintw(win, row, 2, "[%c]  Verbose debug logs",
                      cfg->debug_verbose ? 'X' : ' ');
            break;
        case ITEM_INIT_PATH:
    ...
    while (!done) {
        ...
        case KEY_RIGHT:
            ...
            case ITEM_ARCH:
                ...
                break;
            case ITEM_PRINTK_ENABLE:
                cfg.printk_enabled = !cfg.printk_enabled;
                break;
            case ITEM_USB_ENABLE:
                cfg.usb_enabled = !cfg.usb_enabled;
                break;
            case ITEM_AUDIO_ENABLE:
            ...
            case ITEM_NET_ENABLE:
                ...
                break;
            case ITEM_DEBUG_VERBOSE:
                cfg.debug_verbose = !cfg.debug_verbose;
                break;
            case ITEM_INIT_PATH:
